    ...
    public class Phase
    {
        public static void Begin()
        {
            ...
            switch (mode)
            {
                ...
                case "Multiple logs":
                    ...
                    var files = Directory.GetFiles(folder, "*.maklai"); // Изменено с *.phem на *.maklai
        ...
        private static bool DecryptFile(RSAParameters key, string file)
        {
            try
            {
                ...
                // Изменено удаление 5 символов (.phem) на 7 символов (.maklai)
                using (var fileStream = new FileStream(file.Substring(0, file.Length - 7) + ".zip", FileMode.Create))
                {
